import { ASTNode, Issue } from '../types';

export class PatternDetector {
  detectIssues(elementTree: ASTNode[], nestingDepth: number): Issue[] {
    const issues: Issue[] = [];
  ...
  private analyzeNode(node: ASTNode, issues: Issue[]) {
  ...
  private hasTextContent(node: ASTNode): boolean {
    return node.children.length > 0;
  }

  detectPatterns(elementTree: ASTNode[]): string[] {
  ...
  private detectNodePatterns(node: ASTNode, patterns: Set<string>) {
    // Detect flex usage
